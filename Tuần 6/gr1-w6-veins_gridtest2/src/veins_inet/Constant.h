...
public:
    ...
    static constexpr const char* RSU_SLOWDOWN = "Slow";
...
static char* mergeContentSlow(long Id){
    char *cstr = new char[strlen(Constant::RSU_SLOWDOWN) + 1];
    strcpy(cstr, Constant::RSU_SLOWDOWN);
    std::string str = std::to_string(Id);
    char *new_str = new char[str.length() + 1];
    strcpy(new_str, str.c_str());
    char *ret = new char[strlen(Constant::RSU_SLOWDOWN) + strlen(new_str) + 1];
    strcpy(ret, cstr);
    strcat(ret, new_str);
    return ret;
}
